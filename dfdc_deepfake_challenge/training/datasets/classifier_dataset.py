...
predictor = dlib.shape_predictor('/home/zyn/DeepFake/dfdc_deepfake_challenge/libs/shape_predictor_68_face_landmarks.dat')
...
class DeepFakeClassifierDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.label = np.load(os.path.join(data_path, label))     
        if self.mode == 'train':
            
            self.landmarks = {}
            landmark_path = os.path.join(self.data_root, "landmarks" + ".npy")
            landmarks = np.load(landmark_path, allow_pickle=True)
            landmarks = [item for item in landmarks]
            for landmark in landmarks:
                if landmark[0] not in self.landmarks.keys():
                    self.landmarks[landmark[0]] = landmark[1]
                else:
                    print('error')
    ...
    def __getitem__(self, index: int):
        ...
        while True:
            if self.mode == 'train':
                id, label = self.data[index]
            elif self.mode == 'test':
                id = self.data[index]
            try:
                # if self.mode == "train":
                #     label = np.clip(label, self.label_smoothing, 1 - self.label_smoothing)
                img_name = self.mode + '_' + str(id)+'.jpg'
                img_path = os.path.join(self.data_root, 'image/' + self.mode, img_name)
                ...
                if self.mode == "train" and self.hardcore and not self.rotation:
                    ...
                    if random.random() < 0.7:
                        # landmarks = np.load(landmark_path, allow_pickle=True)
                        landmarks = self.landmarks.get(id)
                ...
                return {"image": image, "labels": np.array((label,)), "img_name": 'train_' + str(id)}
    ...
    def __getitem__(self, index: int):
        ...
        while True:
            ...
            try:
                # if self.mode == "train":
                #     label = np.clip(label, self.label_smoothing, 1 - self.label_smoothing)
                img_name = 'train_' + str(id)+'.jpg'
                img_path = os.path.join(self.data_root, 'image/train', img_name)
                image = cv2.imread(img_path, cv2.IMREAD_COLOR)
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                # mask = np.zeros(image.shape[:2], dtype=np.uint8)
                # diff_path = os.path.join(self.data_root, "diffs", video, img_file[:-4] + "_diff.png")
