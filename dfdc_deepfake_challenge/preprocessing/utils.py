...
def get_original_img_paths(root_dir):
    originals = []
    label_csv_path = os.path.join(root_dir,'train.labels.csv')
    csv = pd.read_csv(label_csv_path, encoding='GBK')
    for i, row in csv.iterrows():
        label = row[0].split('\t')
        if int(label[-1]) == 0: # 0 is real
            originals.append(os.path.join(root_dir, 'image/train', label[0]))
    return originals
